...
const PadComponent = (props) => {
  ...
  const items = keys.map((key) => (<Button key={key} onClick={handler} disabled={power === off} type="button" value={key} variant="dark" className="shadow-sm drum-pad" id={key}>{key}</Button>));
  return (
